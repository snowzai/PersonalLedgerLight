<template>
  <div class="p-4 md:p-8 max-w-6xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2">
        <button @click="store.prevMonth()" class="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 cursor-pointer transition-colors">
        ...
        <span class="text-sm font-semibold text-slate-700 dark:text-slate-300 w-20 text-center tabular-nums">{{ store.currentMonth.replace('-', '年').replace(/(\d{2})$/, '$1月') }}</span>
        <button @click="store.nextMonth()" :disabled="store.isCurrentMonth()" class="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 cursor-pointer transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        >
          <div class="flex items-center gap-2.5 mb-3">
            ...
            <span class="text-sm font-medium text-slate-600 dark:text-slate-300 truncate">{{ l.name }}</span>
            <span v-if="l.excludeFromTotal" class="text-[10px] text-slate-400 bg-slate-100 dark:bg-slate-600 px-1.5 py-0.5 rounded-full flex-shrink-0">不計入</span>
          </div>
          <p class="text-lg font-bold tabular-nums" :class="l.balance >= 0 ? 'text-slate-900 dark:text-slate-100' : 'text-red-500'">
            {{ formatCurrency(l.balance) }}
          </p>
          <div class="mt-1.5 h-1 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-600">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-6">
      <div class="card p-5 lg:col-span-3">
        <h2 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4">近 6 個月收支</h2>
        <apexchart type="bar" height="220" :options="barOptions" :series="barSeries" />
      </div>
      <div class="card p-5 lg:col-span-2">
        <h2 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4">本月支出分類</h2>
    ...
    <!-- Recent items -->
    <div class="card p-5">
      <h2 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-4">最近記錄</h2>
      ...
      <div v-else class="space-y-1">
        ...
        <template v-for="item in store.recentItems" :key="item.id">
          <div
            ...
            class="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors cursor-pointer"
...
const barOptions = computed(() => {
  ...
  return {
    ...
    xaxis: {
      ...
      labels: { style: { colors: dark ? '#94A3B8' : '#94A3B8', fontSize: '12px' } },
    },
    yaxis: { labels: { style: { colors: '#94A3B8', fontSize: '11px' }, formatter: v => v >= 10000 ? (v / 10000).toFixed(0) + 'w' : v.toLocaleString() } },
    grid: { borderColor: dark ? '#334155' : '#F1F5F9', strokeDashArray: 4, xaxis: { lines: { show: false } } },
    legend: { position: 'top', horizontalAlign: 'right', fontSize: '12px', markers: { size: 6 }, labels: { colors: dark ? '#CBD5E1' : undefined } },
    dataLabels: { enabled: false },
    tooltip: { theme: dark ? 'dark' : 'light', y: { formatter: v => 'NT$ ' + v.toLocaleString('zh-TW') }, style: { fontFamily: 'Inter, sans-serif' } },
  }
})
...
const donutOptions = computed(() => {
  ...
  return {
    ...
    legend: { position: 'bottom', fontSize: '11px', markers: { size: 5 }, labels: { colors: dark ? '#CBD5E1' : undefined } },
    dataLabels: { enabled: false },
    plotOptions: { pie: { donut: { size: '65%', labels: { show: true, total: {
      show: true, label: '總支出',
      formatter: () => 'NT$ ' + store.monthExpense.toLocaleString('zh-TW'),
      color: dark ? '#94A3B8' : '#64748B', fontSize: '11px', fontWeight: 600,
    }}}}},
    tooltip: { theme: dark ? 'dark' : 'light', y: { formatter: v => 'NT$ ' + v.toLocaleString('zh-TW') }, style: { fontFamily: 'Inter, sans-serif' } },
    stroke: { width: 2, colors: dark ? ['#1E293B'] : undefined },
  }
})
</script>
