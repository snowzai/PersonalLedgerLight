    ...
    >
      <div class="bg-white dark:bg-slate-800 w-full md:max-w-md rounded-t-3xl md:rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
          <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ isEditing ? '編輯交易' : '新增交易' }}</h2>
          <button
            @click="$emit('close')"
        ...
        <div class="px-6 py-5 space-y-5 overflow-y-auto max-h-[80vh]">
          ...
          <div class="flex bg-slate-100 dark:bg-slate-700 rounded-xl p-1">
            <button
              ...
              :class="form.type === 'expense' ? 'bg-white dark:bg-slate-600 text-red-500 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'"
            >支出</button>
            <button
              @click="form.type = 'income'"
              class="flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 cursor-pointer"
              :class="form.type === 'income' ? 'bg-white dark:bg-slate-600 text-emerald-600 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'"
            >收入</button>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-2" :class="store.ledgers.length > 4 ? 'sm:grid-cols-3' : ''">
              <button
                ...
                :class="form.ledgerId === l.id ? 'border-current shadow-sm' : 'border-transparent bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600'"
              ...
              >
                ...
                <span class="text-xs font-medium text-slate-700 dark:text-slate-300 truncate">{{ l.name }}</span>
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-4 gap-2">
              ...
              >
                ...
                <span class="text-xs font-medium text-slate-600 dark:text-slate-300 leading-tight">{{ cat }}</span>
              </button>
            </div>
          </div>
          ...
          <div>
            <label for="tx-desc" class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">描述</label>
            <input
              id="tx-desc"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label for="tx-date" class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">日期</label>
              <input id="tx-date" v-model="form.date" type="date" :max="today" class="input-field text-sm" />
            </div>
            <div>
              <label for="tx-note" class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">備註</label>
